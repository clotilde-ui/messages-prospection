...
export default function ClientDetail({ client: initialClient, onBack, onSelectAnalysis, onNewAnalysis }: ClientDetailProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <button onClick={onBack} className="flex items-center gap-2 text-[#232323]/60 hover:text-studio-accent mb-8 font-bold uppercase tracking-wider text-xs transition-colors">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          ...
          {client.website_url && (
            <a href={client.website_url} target="_blank" rel="noopener noreferrer" className="text-studio-accent hover:underline font-mono text-sm">
              {client.website_url}
            </a>
          )}
        </div>
        <button onClick={onNewAnalysis} className="flex items-center gap-2 px-6 py-4 bg-studio-accent text-[#232323] hover:bg-studio-accentHover hover:shadow-[4px_4px_0px_0px_#232323] transition-all duration-200 font-bold uppercase text-xs tracking-widest border-2 border-transparent hover:border-[#232323] rounded-none">
          <Plus className="w-5 h-5" /> Nouvelle Analyse
        </button>
      </div>

      {/* SECTION IDENTITÉ VISUELLE : Carte Noire, Texte Beige */}
      <div className="bg-[#232323] p-6 mb-10 border-l-4 border-studio-accent shadow-md group relative rounded-none">
        <div className="flex items-center justify-between mb-6 border-b border-[#FAF5ED]/10 pb-2">
          ...
          {!isEditing ? (
            <button onClick={() => setIsEditing(true)} className="text-[#FAF5ED]/30 hover:text-studio-accent transition-colors p-1" title="Modifier l'identité">
              <Edit2 className="w-4 h-4" />
            </button>
          ) : (
            <div className="flex items-center gap-2">
              <button onClick={saveIdentity} className="text-studio-accent hover:text-studio-accentHover p-1 bg-[#FAF5ED]/10" title="Sauvegarder"><Save className="w-4 h-4" /></button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-fade-in">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <input type="text" value={editForm.primary_color} onChange={(e) => setEditForm({ ...editForm, primary_color: e.target.value })} className="w-full bg-[#2A2A2A] border border-[#3A3A3A] text-[#FAF5ED] px-2 py-1 text-xs font-mono focus:border-studio-accent outline-none rounded-none" />
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <input type="text" value={editForm.secondary_color} onChange={(e) => setEditForm({ ...editForm, secondary_color: e.target.value })} className="w-full bg-[#2A2A2A] border border-[#3A3A3A] text-[#FAF5ED] px-2 py-1 text-xs font-mono focus:border-studio-accent outline-none rounded-none" />
              </div>
            </div>
            <div className="md:col-span-2">
              <label className="block text-[#FAF5ED]/50 text-[10px] font-bold uppercase mb-2">Mood / Ambiance</label>
              <input type="text" value={editForm.brand_mood} onChange={(e) => setEditForm({ ...editForm, brand_mood: e.target.value })} className="w-full bg-[#2A2A2A] border border-[#3A3A3A] text-[#FAF5ED] px-3 py-2 text-sm focus:border-studio-accent outline-none rounded-none" />
            </div>
          </div>
        ) : (
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-[#232323] border border-[#232323] rounded-none">
          ...
          <button onClick={onNewAnalysis} className="text-studio-accent hover:text-[#FAF5ED] font-bold uppercase tracking-wide underline decoration-2 underline-offset-4 transition-colors">
        ...
        <div className="space-y-4">
          {analyses.map((analysis) => {
            ...
            return (
              <div key={analysis.id} onClick={() => onSelectAnalysis(analysis)} className="group flex items-center justify-between p-6 bg-[#232323] border-l-4 border-transparent hover:border-studio-accent hover:translate-x-1 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-lg rounded-none">
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-3 mb-2">
                    {/* Titre Analyse : Beige sur fond noir */}
                    <h3 className="text-lg font-bold text-[#FAF5ED] group-hover:text-studio-accent truncate transition-colors">
                      {analysis.brand_name}
                    </h3>
                  ...
                  <div className="flex items-center gap-6 mt-4">
                    ...
                    <div className="flex items-center gap-4">
                      {videoCount > 0 && (
                        <div className="flex items-center gap-1.5 text-studio-accent text-xs font-bold uppercase tracking-wide">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 bg-[#2A2A2A] flex items-center justify-center group-hover:bg-studio-accent transition-colors rounded-none">
