...
export default function AnalysisForm({ client, onBack, onAnalysisCreated }: AnalysisFormProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-[#232323]/60 hover:text-studio-accent mb-8 font-bold uppercase tracking-wider text-xs transition-colors"
      >
      ...
      <div className="bg-[#232323] p-8 shadow-[8px_8px_0px_0px_#FFBEFA] border-2 border-[#232323]">
        <h2 className="text-3xl font-black text-[#FAF5ED] mb-8 uppercase tracking-tight">
          Nouvelle Analyse <span className="text-studio-accent">/</span> {client.name}
        </h2>
          ...
          <div className="flex gap-0">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-[#2A2A2A] border-2 border-[#3A3A3A] text-[#FAF5ED] focus:border-studio-accent focus:bg-[#232323] placeholder-[#FAF5ED]/20 font-mono text-sm transition-colors"
                disabled={scraped}
              />
            </div>
            <button
              ...
              className="px-8 py-4 bg-studio-accent text-[#232323] hover:bg-studio-accentHover disabled:bg-[#3A3A3A] disabled:text-[#FAF5ED]/20 transition-colors font-bold uppercase tracking-widest text-xs"
            >
              {loading ? (
        ...
        {scraped && (
          <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      value={formData.primaryColor}
                      onChange={(e) => setFormData({ ...formData, primaryColor: e.target.value })}
                      className="flex-1 px-4 py-2 bg-[#232323] border border-[#3A3A3A] text-[#FAF5ED] font-mono text-sm focus:border-studio-accent outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative">
                    <Sparkles className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-studio-accent" />
                    <input
                      type="text"
                      value={formData.brandMood}
                      onChange={(e) => setFormData({ ...formData, brandMood: e.target.value })}
                      className="w-full pl-10 pr-4 py-3 bg-[#232323] border border-[#3A3A3A] text-[#FAF5ED] focus:border-studio-accent outline-none text-sm"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-studio-accent uppercase tracking-wider mb-2">Nom de la marque</label>
                <input
                  type="text"
                  value={formData.brandName}
                  onChange={(e) => setFormData({ ...formData, brandName: e.target.value })}
                  className="w-full px-4 py-3 bg-[#2A2A2A] border-2 border-[#3A3A3A] text-[#FAF5ED] focus:border-studio-accent"
                  required
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-studio-accent uppercase tracking-wider mb-2">Offre / Produits / Services</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-bold text-studio-accent uppercase tracking-wider mb-2">Cibles prioritaires</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-bold text-studio-accent uppercase tracking-wider mb-2">Positionnement</label>
                <textarea
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 px-6 py-4 bg-studio-accent text-[#232323] hover:bg-studio-accentHover disabled:opacity-50 transition-colors font-bold uppercase tracking-widest text-xs shadow-lg"
              >
